--[[ 
    Modern UI Library - Standalone (LuaU)
    Features: 
    - Auto Config System (Save/Load)
    - Forced Settings & Config Tabs
    - User Profile Section
    - Smooth Animations (Open/Close/Hide)
...
function Library:CreateWindow(options)
    ...
    -- Main Container (for Hide/Show animation)
    ...
    -- Bottom Fixed Tabs (Config & Settings)
    ...
    local ProfileImg = Create("ImageLabel", {
        ...
        Image = "rbxassetid://" .. Players:GetUserThumbnailAsync(LocalPlayer.UserId, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size48x48)
    })
    ...
    function Window:Tab(name, icon)
        local Page = Create("ScrollingFrame", {
            ...
            CanvasSize = UDim2.new(0, 0, 0, 0), ScrollBarThickness = 2, Visible = false
        })
        ...
        function Tab:Button(text, callback)
            local BtnFrame = Create("Frame", { Parent = Page, BackgroundColor3 = Library.CurrentTheme.Secondary, Size = UDim2.new(1, 0, 0, 35) })
        ...
        function Tab:Toggle(text, default, flag, callback)
            ...
            local ToggleFrame = Create("Frame", { Parent = Page, BackgroundColor3 = Library.CurrentTheme.Secondary, Size = UDim2.new(1, 0, 0, 35) })
            ...
            local function Update()
                ...
                Library:Notify("Toggle", text .. ": " .. tostring(Toggled), 1)
            end
        ...
        function Tab:Slider(text, min, max, default, flag, callback)
            Library.Flags[flag] = default
            local SliderFrame = Create("Frame", { Parent = Page, BackgroundColor3 = Library.CurrentTheme.Secondary, Size = UDim2.new(1, 0, 0, 50) })
        ...
        function Tab:Dropdown(text, options, default, flag, callback)
            Library.Flags[flag] = default
            local Dropped = false
            local DropFrame = Create("Frame", { Parent = Page, BackgroundColor3 = Library.CurrentTheme.Secondary, Size = UDim2.new(1, 0, 0, 35), ClipsDescendants = true, ZIndex = 2 })
            ...
            local List = Create("ScrollingFrame", { Parent = DropFrame, BackgroundTransparency = 1, Position = UDim2.new(0, 0, 0, 35), Size = UDim2.new(1, 0, 0, 100), CanvasSize = UDim2.new(0, 0, 0, 0), ScrollBarThickness = 2 })
        ...
        function Tab:Input(text, placeholder, flag, callback)
            Library.Flags[flag] = ""
            local InputFrame = Create("Frame", { Parent = Page, BackgroundColor3 = Library.CurrentTheme.Secondary, Size = UDim2.new(1, 0, 0, 35) })
        ...
        function Tab:Keybind(text, default, flag, callback)
            Library.Flags[flag] = default
            local Key = default
            local KeyFrame = Create("Frame", { Parent = Page, BackgroundColor3 = Library.CurrentTheme.Secondary, Size = UDim2.new(1, 0, 0, 35) })
        ...
        -- Visual Components
        function Tab:Skeleton()
            local Skel = Create("Frame", { Parent = Page, BackgroundColor3 = Color3.fromRGB(40,40,40), Size = UDim2.new(1, 0, 0, 60) })
            Create("UICorner", { CornerRadius = UDim.new(0, 6), Parent = Skel })
            local Gradient = Create("UIGradient", { Parent = Skel, Color = ColorSequence.new{ColorSequenceKeypoint.new(0, Color3.new(1,1,1)), ColorSequenceKeypoint.new(0.5, Color3.new(0.8,0.8,0.8)), ColorSequenceKeypoint.new(1, Color3.new(1,1,1))}, Rotation = 45 })
            task.spawn(function()
                while Skel.Parent do
                    Tween(Gradient, TweenInfo.new(1, Enum.EasingStyle.Linear, Enum.EasingDirection.InOut, -1, true), {Offset = Vector2.new(1, 0)})
                    task.wait(2)
                end
            end)
        end
        ...
        function Tab:Card(title, content)
            local Card = Create("Frame", { Parent = Page, BackgroundColor3 = Library.CurrentTheme.Secondary, Size = UDim2.new(1, 0, 0, 80) })
    ...
    local function CreateBottomTab(name)
        ...
        -- Return Tab Object (Simplified)
        local Tab = {}
        function Tab:Dropdown(...) return Window.Tab(Window, "Temp").Dropdown(Tab, ...) end -- Reuse logic
        function Tab:Button(...) return Window.Tab(Window, "Temp").Button(Tab, ...) end
        function Tab:Input(...) return Window.Tab(Window, "Temp").Input(Tab, ...) end
        function Tab:Toggle(...) return Window.Tab(Window, "Temp").Toggle(Tab, ...) end
        function Tab:Keybind(...) return Window.Tab(Window, "Temp").Keybind(Tab, ...) end
        return Tab
    end
    ...
    ConfigTab:Button("Load Config", function()
        if isfile(Library.FolderName .. "/" .. ConfigName .. ".json") then
            ...
            for flag, val in pairs(data) do
                Library.Flags[flag] = val
                if Library.Flags[flag.."_Object"] and Library.Flags[flag.."_Object"].Set then
                    Library.Flags[flag.."_Object"]:Set(val)
                end
            end
